...
class CountFunctionV2: public FlatMapFunction<Tuple2V2<const char*, int, 128, sizeof(int)>, Tuple2V2<const char*, int, 128, sizeof(int)>>{
private:
    std::map<std::string, int> m_counter;
public:
    void flat_map(
    ...
    std::shared_ptr<FlatMapFunction<Tuple2V2<const char*, int, 128, sizeof(int)>, Tuple2V2<const char*, int, 128, sizeof(int)>>> 
          deserialize() override { return std::make_shared<CountFunctionV2>();}
};
...
public:
    void testWordCount( void ) {
        ...
        std::shared_ptr<StreamOperator<MyTuple>> stream_counter = 
                                            std::make_shared<StreamFlatMap<MyTuple, MyTuple>>(std::make_shared<CountFunctionV2>());
        ...
        std::shared_ptr<StreamEdge<MyTuple>> edge_2 = std::make_shared<StreamEdge<MyTuple>>(node_2, node_4, std::make_shared<ForwardPartitioner<MyTuple>>());

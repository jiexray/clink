...
public:
    ...
    void flat_map(std::shared_ptr<std::string> value, std::shared_ptr<Collector<Tuple2<std::string, int>>> collector) {
        ...
        while (true) {
            int next_pos = value->find(' ', pos);

            if (next_pos == std::string::npos) {
                // get to the end
                collector->collect(std::make_shared<Tuple2<std::string, int>>(
                                                            std::make_shared<std::string>(*value, pos, value->size() - pos),
                                                            std::make_shared<int>(1)));
                break;
            } else {
                collector->collect(std::make_shared<Tuple2<std::string, int>>(
                                                            std::make_shared<std::string>(*value, pos, next_pos - pos),
                                                            std::make_shared<int>(1)));
...
public:
    void testWordCount( void ) {
        ...
        std::shared_ptr<StreamEdge<MyTuple>> edge_3 = std::make_shared<StreamEdge<MyTuple>>(node_2, node_5, std::make_shared<ForwardPartitioner<MyTuple>>());

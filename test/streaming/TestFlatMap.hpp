...
class MyFlatMapFunction: public FlatMapFunction<std::string, std::string> {
    void flat_map(std::shared_ptr<std::string> value, std::shared_ptr<Collector<std::string>> collector) {
        for (size_t i = 0; i < value->size(); i++) {
            collector->collect(std::make_shared<std::string>(value->begin() + i, value->begin() + i + 1));
        }        
    }

    char* serialize() override {return (char*)this;}

    std::shared_ptr<FlatMapFunction<std::string, std::string>> deserialize() override { return std::make_shared<MyFlatMapFunction>();}
};
...
public:
    void testStreamFlatMap() {
        ...
        std::shared_ptr<StreamOperator<std::string>> stream_flat_map = std::make_shared<StreamFlatMap<std::string, std::string>>(std::make_shared<MyFlatMapFunction>());

...
private:
    ...
    std::shared_ptr<TypeDeserializerImpl>                   m_current_record_deserializer;
    std::vector<std::shared_ptr<TypeDeserializerImpl>>      m_record_deserializers;
...
public:
    ...
    StreamTaskNetworkInput(std::shared_ptr<InputGate> input_gate, int input_idx){
        ...
        for (int i = 0; i < num_input_channels; i++) {
            m_record_deserializers.push_back(std::make_shared<TypeDeserializerImpl>());
        }
    }
    void                                                    close() {
...
class StreamTaskNetworkInput<T<T0, T1>> : public StreamTaskInput<T<T0, T1>>
{
private:
    std::shared_ptr<InputGate>                              m_input_gate;
    int                                                     m_last_channel; // the last input channel, which return by poll_next() of InputGate
    int                                                     m_input_idx; // identify the index of an input gate if there are multiple input gates
                                                                         // locate in a single StreamTask.
                                                                         // Currently, only one input gate supported
    std::shared_ptr<StreamRecord<T<T0, T1>>>                m_current_record;
    std::shared_ptr<IOReadableWritable>                     m_deserialization_delegate;

    /* deserialization array of all input channels */
    std::shared_ptr<TupleDeserializerV2>                    m_current_record_deserializer;
    std::vector<std::shared_ptr<TupleDeserializerV2>>         m_record_deserializers;
    ...
    static std::shared_ptr<spdlog::logger>                  m_logger;
    int                                                     m_records_cnt = 0;
    double                                                  m_average_record_serialize_time = 0;
...
public:
    StreamTaskNetworkInput(std::shared_ptr<InputGate> input_gate, int input_idx, std::shared_ptr<IOReadableWritable> deserialization_delegate, RECORD_TYPE record_type) {
        ...
        for (int i = 0; i < num_input_channels; i++) {
            m_record_deserializers.push_back(std::make_shared<TupleDeserializerV2>());
        }
    }
    ...
    /* emit input data record */
    InputStatus                                             emit_next(std::shared_ptr<DataOutput<T<T0, T1>>> output){
        while (true)
        {
            if (this->m_current_record_deserializer != nullptr) {
                // use a delegate to get the data from deserializer
                ...
                if (result == DeserializationResult::INTERMEDIATE_RECORD_FROM_BUFFER || result == DeserializationResult::LAST_RECORD_FROM_BUFFER) {
                    ...
                    std::shared_ptr<StreamRecord<T<T0, T1>>> new_record = std::make_shared<StreamRecord<T<T0, T1>>>(
                                                                    std::dynamic_pointer_cast<TupleDeserializationDelegateV2<T<T0, T1>>>(m_deserialization_delegate)->get_instance());
                    output->emit_record(new_record);
...
template<template <class, class> class T, class T0, class T1>
std::shared_ptr<spdlog::logger> StreamTaskNetworkInput<T<T0, T1>>::m_logger = LoggerFactory::get_logger("StreamTaskNetworkInput");
...
void InputGateFactory::create_input_channels(std::string owning_task_name, std::shared_ptr<InputGateDeploymentDescriptor> igdd, std::shared_ptr<InputGate> input_gate) {
    ...
    for (int i = 0; i < num_input_channels; i++) {
        input_channels[i] = create_input_channel(input_gate, i, partition_ids[i]);
    }

    input_gate->set_input_channels(input_channels, num_input_channels);
}

std::shared_ptr<InputChannel> InputGateFactory::create_input_channel(std::shared_ptr<InputGate> input_gate, int input_channel_idx, std::string partition_id) {
    return std::make_shared<InputChannel>(input_gate, input_channel_idx, partition_id, m_partition_manager);
}
